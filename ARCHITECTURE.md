...
## External Dependencies & Build Modes

GraphStudio's own server depends on six crates from the private
`rust-shared-utils` repository — `pipeline` (ETL engine), `pg` (Postgres pools),
`app-config` (config types), `rcl` (Retail Constraint Language rules engine),
`cdc` (Postgres change-data-capture), and `secret_manager` (GCP Secret Manager).
These are the only external/private dependencies; everything else comes from
public registries.

The repo builds in one of two modes, selected per-dependency in
`server/Cargo.toml`:

- **Scaffold-stub build (default, public).** The six crates resolve to inert
  local stub crates under `server/stubs/`. The server compiles and boots with
  **no SSH/Bitbucket access and no proprietary code** — but the features those
  crates back are inert: no pipeline execution, no live PG pools, CDC disabled
  (a start attempt reports failure instead of a false "streaming" status), no
  GCP secrets, and **`rcl` carries none of its resolution algorithm** (it
  resolves to empty). This is a scaffold for experimentation, not a functional
  deployment. See `server/stubs/README.md`.
- **Real-crate build.** Each dependency's original `git = "ssh://…"` line (kept
  commented directly above its `path = "stubs/…"` line) is restored, and the
  build pulls the real crates over SSH — requiring Bitbucket read access.

The live build mode of each crate is reflected in the auto-generated
[Codebase Map](#codebase-map-auto-generated) at the end of this document.
...
| **Shared Utils** | rust-shared-utils (`pipeline`, `pg`, `app-config`, `rcl`, `cdc`, `secret_manager`) | Private Rust crates the server depends on — replaced by inert local stubs in the public build (see [External Dependencies & Build Modes](#external-dependencies--build-modes)) |
...
### Backend modules (`server/src/`) — 21

`agent` · `app_state` · `article_selection` · `clickhouse` · `cross_filter` · `db` · `db_config` · `graph` · `handlers` · `instance_config` · `pg_pools` · `pipeline` · `pipeline_assemblies` · `proc_registry` · `query` · `router` · `seed` · `service` · `services` · `trace_db` · `uam`

### HTTP API surface (all `server/src` routers) — 131 routes across 33 groups
...
### External shared-crate dependencies (`rust-shared-utils`)

| Crate | Build mode |
|---|---|
| `pipeline` | stub (path) |
| `pg` | stub (path) |
| `app-config` | stub (path) |
| `rcl` | stub (path) |
| `cdc` | stub (path) |
| `secret_manager` | stub (path) |
